...
% Create a simulink bus elements for Black start
BlackStartElements = {'enable', 'status', 'blackout'};
BlackStart = Simulink.Bus;
for elementIdx = 1 : length(BlackStartElements)
    BlackStart.Elements(elementIdx) = Simulink.BusElement;
    BlackStart.Elements(elementIdx).Name = BlackStartElements{elementIdx};
    BlackStart.Elements(elementIdx).Complexity = 'real';
    BlackStart.Elements(elementIdx).Dimensions = 1;
    BlackStart.Elements(elementIdx).DataType = 'boolean';
    BlackStart.Elements(elementIdx).Min = [];
    BlackStart.Elements(elementIdx).Max = [];
    BlackStart.Elements(elementIdx).DimensionsMode = 'Fixed';
    BlackStart.Elements(elementIdx).SamplingMode = 'Sample based';
    BlackStart.Elements(elementIdx).SampleTime = -1;
    BlackStart.Elements(elementIdx).DocUnits = '';
    BlackStart.Elements(elementIdx).Description = '';
end
...
StatusElements = {'mode','resync','plannedIslanding','blackStart','resyncEnable','plannedIslandingEnable','blackStartEnable','bessMode','motorBreaker'};
Status = Simulink.Bus;
booleanIdx = [2,3,4,5,6,7];
enumIdx = 1;
for elementIdx = 1 : length(StatusElements)
    ...
    elseif any(elementIdx == enumIdx)
        Status.Elements(elementIdx).DataType = 'Enum: Transition_and_Dispatch_ControlModeType';
    else
