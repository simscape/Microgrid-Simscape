...
if ~exist('h4_remote_microgrid', 'var') || ...
        ~isgraphics(h4_remote_microgrid, 'figure')
    h4_remote_microgrid = figure('Name', 'remote_microgrid');
end
figure(h4_remote_microgrid)
clf(h4_remote_microgrid)

frequencyLimit=Microgrid.FrequencyLimit;
voltageLimit=Microgrid.VoltageLimit;
plotRemoteMicrogridFreAngle(logsout_remote_microgrid, caseNum,frequencyLimit,voltageLimit)

% Plot microgrid planned island

function plotRemoteMicrogridFreAngle(logsout, caseNum,frequencyLimit,voltageLimit)
...
simlog_handles(1) = subplot(4, 1, 4);
